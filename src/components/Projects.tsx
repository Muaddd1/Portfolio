...
function isLiveDemo(url: string): boolean {
  return url.length > 0 && !url.includes('gumroad') && !url.includes('vercel.app/') && !url.includes('muad-portfolio');
}
...
function ProjectCard({ project, index }: { project: typeof projects[0]; index: number }) {
  const hasLiveDemo = isLiveDemo(project.demo);
  const isGumroad = project.demo.includes('gumroad');
    ...
    >
      ...
      <div className="relative overflow-hidden rounded-xl bg-white/[0.02] border border-white/[0.06] hover:border-cyan/15 transition-all duration-500">
        ...
        <div className="relative aspect-[16/9] overflow-hidden">
          ...
          <div className="absolute inset-0 bg-void/60 opacity-0 group-hover:opacity-100 transition-opacity duration-400 flex items-center justify-center gap-5">
            {hasLiveDemo && (
              <a
                href={project.demo}
                target="_blank"
